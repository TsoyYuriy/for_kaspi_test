...
Перед запуском проекта необходима заполнить базу данных. В корне проекта находится postman коллекция с роутами для создания(KASPI_API.postman_collection), если не хотите заполнять вручную то можно взять пример из 4 пункта(запрос всех постов), и всавить.
